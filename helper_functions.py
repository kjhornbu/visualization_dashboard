...
def use_config_on_Data():
    ...
    if myPlotConfig.filter is not None:
        Reduced_Stats=filter_stat_sheet(myPlotConfig.filter,Group_Stats.data)
    else: 
...
def collect_from_data(sheet):
    ...
    if myPlotConfig.use_sheet == 'group':
        merged_data = pd.merge(sheet, Group_Data, on=myPlotConfig.x, how='inner',copy=False,suffixes= ("", "_delete_me"))
    elif myPlotConfig.use_sheet =='indiv':
        merged_data = pd.merge(sheet, Indiv_Data, on=myPlotConfig.x, how='inner',copy=False,suffixes= ("", "_delete_me"))
...
def reduce_to_top_Data():  
    ...
    if myPlotConfig.use_sheet == 'group':
        data_pivot = pd.pivot_table(Group_Data.data, values=myPlotConfig.y, index=myPlotConfig.x, columns=list(Group_Data.groupings.keys()))
    elif myPlotConfig.use_sheet =='indiv':
        data_pivot = pd.pivot_table(Indiv_Data.data, values=myPlotConfig.y, index=myPlotConfig.x, columns=list(Indiv_Data.groupings.keys()))
