...
class PlotConfig: 
    def __init__(self,use_sheet:str=None,x:str=None,y:str=None,groups_to_include:dict={},config_reducereorder:dict={},config_filter:dict={}):
        ...
        self.x = x
        self.y = y
        self.groups_to_include=groups_to_include #-> None
    ...
    def isValid(self):
        if self.use_sheet is not None and self.x is not None and self.y is not None and len(self.reduce_reorder)>0 and len(self.filter)>0:
            isValid = True
        else: 
            isValid = False
